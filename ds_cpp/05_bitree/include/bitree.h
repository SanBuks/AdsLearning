...
class BiTree {
 ...
 protected:
  ...
  // 是否是根节点
  inline bool IsRoot(BNP p) const;
  // 是否是左孩子
  inline bool IsLc(BNP p) const;
  // 是否是右孩子
  inline bool IsRc(BNP p) const;
  // 是否有父节点
  inline bool HasParent(BNP p) const;
  // 是否有左孩子
  inline bool HasLc(BNP p) const;
  // 是否有右孩子
  inline bool HasRc(BNP p) const;
  // 是否有孩子
  inline bool HasC(BNP p) const;
  // 是否是叶子节点
  inline bool IsLeaf(BNP p) const;
  // 是否有两个孩子
  inline bool HasBc(BNP p) const;
...
template <typename T>
inline bool BiTree<T>::IsRoot(BNP p) const {
  return p && !p->parent_;
}

template <typename T>
inline bool BiTree<T>::IsLc(BNP p) const {
  return p && !IsRoot(p) && p->parent_->lc_ == p;
}

template <typename T>
inline bool BiTree<T>::IsRc(BNP p) const {
  return p && !IsRoot(p) && p->parent_->rc_ == p;
}

template <typename T>
inline bool BiTree<T>::HasParent(BNP p) const {
  return p && !IsRoot(p);
}

template <typename T>
inline bool BiTree<T>::HasLc(BNP p) const {
  return p && p->lc_;
}

template <typename T>
inline bool BiTree<T>::HasRc(BNP p) const {
  return p && p->rc_;
}

template <typename T>
inline bool BiTree<T>::HasC(BNP p) const {
  return p && (HasLc(p) || HasRc(p));
}

template <typename T>
inline bool BiTree<T>::IsLeaf(BNP p) const {
  return p && !HasC(p);
}

template <typename T>
inline bool BiTree<T>::HasBc(BNP p) const {
  return p && HasLc(p) && HasRc(p);
}
